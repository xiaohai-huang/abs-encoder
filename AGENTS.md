...
- `i2c_pos.c/.h` — I2C slave host port: the decoded position as one combined 27-bit count (`turns·16384 + angle`) plus a status/valid byte, pure logic (no HAL). The snapshot is packed into one 32-bit word so an address-match read can never tear; wire protocol in `docs/i2c.md`
